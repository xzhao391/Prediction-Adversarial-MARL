...
def RVO_update(X, V_des, V_current):
    ...
    V_opt = [np.array([0, 0]), np.array([0, 0]), np.array([0, 0])]
    p_force = []
    for i in range(len(X)):
        ...
        # acc = np.linalg.norm(V_opt[i] - v_old[i]) # Magnitude of desired acceleration
        # steering = math.atan2(V_opt[i][1] - v_old[i][1], V_opt[i][0] - v_old[i][0])
        v_current = V_current[i]
        v_target = V_opt[i]
        speed_current = np.linalg.norm(v_current)
        speed_target = np.linalg.norm(v_target)

        acc = speed_target - speed_current
        heading = math.atan2(v_current[1], v_current[0])
        if speed_target > 3e-3:
            heading_target = math.atan2(v_target[1], v_target[0])
        else:
            heading_target = heading  # default to current heading

        delta_heading = wrap_to_pi(heading_target - heading)
        if speed_target > 1e-2:
            wheelbase = 1
            steering = math.atan((wheelbase * delta_heading) / speed_target)
        else:
            steering = 0.0
        p_force.append([acc + np.random.rand() / 100 , steering + np.random.rand() / 100])
    return p_force
